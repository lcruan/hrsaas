...
module.exports = {
  ...
  devServer: {
    ...
    proxy: {
      // 当地址中有/api的时候会触发代理机制
      '/api': {
        target: 'http://ihrm-java.itheima.net/', // 要代理的服务器地址 这里不用写api 会把api自动加在网址后面
        changeOrigin: true
        // 重写路径
        // pathRewrite: {}
      }
  ...
  configureWebpack: {
    ...
    // 因为要排除elementui 所以后面要引入CDN文件 cdn文件中有elementui的全局变量名
  ...
  chainWebpack(config) {
    ...
    // 注入cdn变量
    // 这样代码会在 执行打包的时候 执行  就会将cdn变量注入到html模板中
